...
function MovieCategory({ category }) {
   ...
   /* 
      ...
      메뉴 클릭시 MovieCategory 컴포넌트는 재렌더링 X 

      -> 같은 MovieCategory 컴포넌트를 사용하기 때문에 react-router-dom에서 경로가 바뀌어도 새로운 컴포넌트를 렌더링 하지 X
      
      -> 따라서 state는 최초로 메뉴중 하나를 클릭시 처음에만 1로 지정되고 이후 다른 메뉴 클릭시 1로 최기화 되지 않음

      -> 따라서 반드시 useEffect를 사용해 1로 초기화 

      -> 메인페이지에 있다가 최초로 메뉴를 클릭했을때는 1번 useEffectdhk 2번 useEffect를 모두 실행한다

      -> 1번 useEffect에서 page 바꿈 2번 useEffect API calling이 모두 일어나고 page가 바뀌는 현상때문에 2번 useEffect API calling이 한번 더 발생한다

      -> 이후 다른 메뉴 클릭시에는 category props만 바뀔 뿐 MovieCategory 컴포넌트가 재렌더링이 되지는 않으므로 API calling이 2번 되는 현상은 발생하지 X 
   ...
   */
   ...
   // category: category 에서 뒤 카테고리는 function MovieCategory({ category })의 category다
   // 2번 useEffect
   useEffect(() => {
